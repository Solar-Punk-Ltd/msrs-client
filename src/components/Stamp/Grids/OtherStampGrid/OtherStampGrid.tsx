...
import { OtherStampWithInfo } from '@/hooks/useOtherStamps';
...
import { StampActions } from '../../Cards/Shared/StampActions';

import './OtherStampGrid.scss';

interface OtherStampGridProps {
  title: string;
...
export function OtherStampGrid({ title, stamps, signer, onStampRefresh }: OtherStampGridProps) {
  return (
      ...
      <div className="stamp-manager-grid">
        {stamps.map((stamp) => (
          <div key={stamp.stampId} className="stamp-wrapper">
            <OtherStampCard stamp={stamp} signer={signer || undefined} onStampRefresh={onStampRefresh} />
          </div>
        ))}
      </div>
    </div>
...
interface OtherStampCardProps {
  stamp: OtherStampWithInfo;
  signer?: ethers.Signer;
  onStampRefresh?: (stampId: string) => Promise<void>;
}

function OtherStampCard({ stamp, signer, onStampRefresh }: OtherStampCardProps) {
